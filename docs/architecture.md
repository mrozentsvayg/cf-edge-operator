...
## Policy resolution (per-feature defaults)

`--management-policy` and `--delete-policy` are the operator-wide defaults. Each feature can override them with a per-feature flag, so one cluster can run CustomHostname and load balancing under different defaults -- e.g. CustomHostname in `create` (coexist with external-dns) while load balancing enforces single-owner `manage`. The effective policy for a CR is resolved highest-precedence-first:

1. **Per-CR** `spec.managementPolicy` / `spec.deletePolicy`.
2. **Per-feature flag** -- `--customhostname-management-policy` / `--customhostname-delete-policy` for CustomHostname; `--loadbalancing-management-policy` / `--loadbalancing-delete-policy` for LoadBalancer, LoadBalancerPool, and LoadBalancerMonitor.
3. **Global flag** -- `--management-policy` / `--delete-policy`.
4. **Built-in default** -- `manage` / `always`.

All four per-feature flags default to empty (inherit the matching global), so the operator binary is symmetric and the global flags stay truly global. The single-owner default for load balancing is a Helm chart opinion, not baked into the binary: the chart sets `loadbalancingManagementPolicy: "manage"` (overridable -- set `""` to inherit the global `--management-policy`), because the LB resources are owned by exactly one control cluster, so `create` / `observe` coexistence does not apply. Account CRs take no policy -- the Account controller only validates credentials. The operator resolves the effective per-feature values, logs them at startup (the `Configuration` line), and rejects an invalid per-feature value at startup, naming the offending flag. See [operations.md](operations.md#policy-precedence-management-and-delete) for the flag reference.
...
**Map-valued field removal.** Cloudflare DEEP-MERGES map-valued fields on PATCH (verified live), so re-sending only the desired set leaves a key the CR dropped lingering -- it must be removed explicitly. The mechanism differs by endpoint. The monitor `header` and `random_steering.pool_weights` endpoints ACCEPT a per-key JSON null, so a dropped key is removed in place by sending the desired keys plus an explicit null for every key Cloudflare still holds (`headerEditOverride` / `mapWithNulls` via `option.WithJSONSet`). The LB geo maps `regionPools` / `countryPools` / `popPools` REJECT a per-key null or empty-array (HTTP 400 code 1002 "the region_pools array length must be in range [1, 16]"), so a dropped geo key is removed by first clearing the whole affected map with a top-level null in a minimal pre-edit PATCH (`clearRemovedGeoKeys`) and then re-adding the remaining keys via the main edit's deep-merge -- two PATCH calls, with a brief window during a removal where the affected map is empty (its traffic falls back to `default_pools`). A full PUT/replace would remove a dropped key in one call but is deliberately avoided: it would wipe the un-modeled Cloudflare LB config (e.g. rules) the PATCH-partial model preserves.

**Policy semantics.** `managementPolicy` (`manage` / `create` / `observe`) and `deletePolicy` (`always` / `own-only` / `never`), including per-CR overrides, apply to the LB resources exactly as they do to CustomHostname. Their default is resolved per feature: the per-feature flags inherit the global `--management-policy` / `--delete-policy` when empty, and the Helm chart sets `loadbalancingManagementPolicy: "manage"` as the single-owner deployment default (overridable -- set `""` to inherit). See [Policy resolution](#policy-resolution-per-feature-defaults), [Management Policy](#management-policy), and [Delete Policy](#delete-policy). Teardown ordering and finalizer-wedge recovery are covered in [operations.md](operations.md#uninstallation).

**Entitlement- and tier-gated Cloudflare features.** Some modeled fields require a Cloudflare account entitlement the operator cannot provision itself: the operator sends them faithfully and Cloudflare enforces the gate at the API. The advanced steering policies (`geo`, `dynamic_latency`, `proximity`, `least_outstanding_requests`, `least_connections`) depend on the traffic-steering add-on / a higher plan tier -- on a base-tier account only `steeringPolicy=off` is available and an advanced policy fails LB create/edit. `proximity` steering additionally requires every participating pool to carry `latitude` / `longitude`; without them Cloudflare rejects the write (HTTP 400 code 1002). None of these are blocked at admission -- there is no CEL rule for account entitlements or for proximity coordinates -- so a missing entitlement or coordinate is not a rejected `kubectl apply` but a reconcile-time failure: a `CreateFailed` / `UpdateFailed` `Ready` condition, an incremented `consecutiveErrors`, and the `cf_edge_operator_loadbalancers{state="error"}` series (which drives the `CfEdgeOperatorLoadBalancersError` alert).

### Unsupported / out-of-scope Cloudflare Load Balancing features

The operator models the writable fields of Cloudflare Load Balancers, Pools, and Monitors. The Cloudflare features below are intentionally not managed. All are coexist-safe: the operator never sends them, so any value configured out-of-band (dashboard / API / Terraform) is preserved across reconciles. Items already covered above are cross-referenced rather than re-explained.

**Already covered above (cross-reference).** The partial-PATCH model leaves un-modeled Cloudflare config intact -- most notably Load Balancer rules (the traffic-steering rule engine), which the operator neither creates nor drift-corrects -- and never force-clears an optional field the CR leaves unset (for example `description`, `notificationEmail`, `minimumOrigins`, and `loadShedding` subfields; `checkRegions` likewise cannot be reset back to all-regions once set). See "Curated subset via PATCH" above. `spec.networks` is create-only-write, with post-create drift surfaced but not auto-corrected -- see "Networks: create-only-write" above.

**Not modeled anywhere.**
- **Monitor groups** (`pool.monitor_group`) -- deferred to a future `LoadBalancerMonitorGroup` CRD plus a `monitorGroupRef` field (a documented TODO on the Pool API type). A pool references at most one `monitorRef` today, or none; the monitor-group and bulk monitor-group endpoints are not modeled.
- **`notification_filter`** (pool/origin health-notification filtering) -- the alerting plane is out of scope.
- **Monitor Preview** (ad-hoc probe preview) and **Monitor References** (which pools use a monitor) -- Cloudflare action/read endpoints, not exposed as CRD actions or mirrored into status; use the Cloudflare dashboard / API.
- **User-Agent probe header** -- the monitor `header` map is free-form, but Cloudflare does not allow overriding the probe's User-Agent, so a `User-Agent` entry has no effect.
- **Pool-level `networks`** -- modeled only on the LoadBalancer CRD; the Pool CRD has no networks field, so any pool-level networks value set out-of-band is preserved.
- **Read-only metadata** (`created_on`, `modified_on`, `zone_name`, `disabled_at`, and per-origin `disabled_at`) -- Cloudflare-computed response fields, not mirrored into CR status (status surfaces `status.id` and, where applicable, `status.enabled` / `status.monitorID`, operational counts, and conditions).

**PoP (`pop_pools`) geosteering: implemented but not live-verified.** Exercising PoP geosteering requires a non-self-serve Cloudflare PoP-geosteering entitlement. Region (`regionPools`) and country (`countryPools`) geosteering are verified against live Cloudflare; `popPools` shares their code path -- including the geo-key removal handling in "Map-valued field removal" above -- but its behavior is inferred from that shared path, not independently tested on live Cloudflare.
